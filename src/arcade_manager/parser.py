...
class Rule:
    ...
    def safe_name(self) -> str:
        ...
        for char in self.name:
            if char in illegal:
                ...
                print(
                    f"Rule {self.name} contains illegal character '{char}', replacing with '_'",
                    severity="WARNING",
                )
        return safe_name

    @property
    def translated_type(self) -> Literal["CALCULATION", "VALIDATION", "CONSTRAINT"]:
    ...
    @property
    def translated_events(
        self,
    ) -> list[Literal["INSERT", "DELETE", "UPDATE"]] | str | None:
        events = []
    ...
    def _convert_flag(self, flag: int) -> Optional[int]:
        if flag < 0:
            return None
        else:
            return flag
    ...
    def commit(
        self, parent_path: Path, existing: dict[int, Rule], delete=False
    ) -> bool:
        parent_path = Path(parent_path)
        parent_name = parent_path.name
...
class Dataset:
    ...
    _relpath: Optional[str | Path] = None
...
class Extractor:
    ...
    def _read(self, schema: dict[str, Any]) -> Dataset:
        return Dataset(
            name=schema["name"],
            datasets=(
                [self._read(dataset) for dataset in schema["datasets"]]
            ...
            rules=(
                [
                    ...
                    for rule in schema["attributeRules"]
                ]
                if "attributeRules" in schema
                else None
            ),
    ...
    def extract(self):
        ...
        print(
            f"Extracted rules from {self.database.name} to {self.repo.name}",
            severity="INFO",
        )
    ...
    def _load(self, path: Path) -> Dataset:
        ...
        return Dataset(
            ...
            datasets=[
                ...
                and not any(
                    item.is_file() for item in dataset.iterdir()
                )  # With no files
            ],
            rules=[
